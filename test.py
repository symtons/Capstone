...
if __name__ == "__main__":
    if not os.path.exists('OpenKBP_C3D'):
        raise Exception('OpenKBP_C3D should be prepared before testing, please run prepare_OpenKBP_C3D.py')

    parser = argparse.ArgumentParser()
    parser.add_argument('--GPU_id', type=int, default=0,
                        help='GPU id used for testing (default: 0)')
    parser.add_argument('--model_path', type=str, default='New_m/best_val_evaluation_index.pkl')
    parser.add_argument('--TTA', type=bool, default=False,
                        help='do test-time augmentation, default True')
    parser.add_argument('--model', type=str, default='Model_MT',
                        help='model to use: "Model" or "MambaModel" (default: "MambaModel")')
    parser.add_argument('--project_name', type=str, default='New_l',
                        help='project name (default: "New_l")')
    ...
    if args.model == 'Model':
        ...
        trainer.setting.network = Model_MTA5(in_ch=9, out_ch=1,
                  list_ch_A=[-1, 16, 32, 64, 128, 256],
                  list_ch_B=[-1, 32, 64, 128, 256, 512],
                  d_state=16, d_conv=4, expand=2, channel_token=False)
    ...
    # Evaluation
    print('\n\n# Start evaluation !')
    list_patient_ids = ['pt_' + str(i) for i in range(241, 341)]
