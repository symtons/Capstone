...
class ParallelConvBlock3D(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_sizes, stride, norm_op, nonlin):
        super(ParallelConvBlock3D, self).__init__()
        assert len(kernel_sizes) > 0, "kernel_sizes must be a non-empty list"

        self.branches = nn.ModuleList([
            nn.Sequential(
                nn.Conv3d(input_channels, output_channels, kernel_size=k, stride=stride, padding=k // 2),
                norm_op(output_channels),
                nonlin()
            )
...
class ResidualMambaBlock(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_sizes, stride, norm_op, nonlin,
        ...
        self.block1 = ParallelConvBlock3D(
        ...
        self.block2 = ParallelConvBlock3D(
            input_channels=input_channels + output_channels,
            output_channels=output_channels,
            kernel_sizes=kernel_sizes,
            stride=1,  # keep stride=1 for next blocks
            norm_op=norm_op,
            nonlin=nonlin
        )
    ...
    def forward(self, x):
        ...
        #x3 = torch.cat([x, out1, out2], dim=1)
        #out3 = self.block3(x3)

        print("After Dense Parallel Blocks:", out2.shape)

        

        if self.se_block:
            out = self.se_block(out2)
...
if __name__ == '__main__':
    model = Model_Den(in_ch=9, out_ch=1,
