...
class CrossAttentionFusionModule(nn.Module):
    ...
    def cross_attention(self, query, key_value):
        ...
        query_features = self.query_conv(query)
        key_features = self.key_conv(key_value)
        value_features = self.value_conv(key_value)

        # Compute attention scores using simple dot product between query and key
        attention_weights = F.softmax(torch.sum(query_features * key_features, dim=1, keepdim=True), dim=-1)
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        input = self.cross_att(CT, PTV, OAR)

        out_encoder_1 = self.encoder_1(input)
        out_encoder_2 = self.encoder_2(self.comb2(out_encoder_1, input))
        out_encoder_3 = self.encoder_3(self.comb3(out_encoder_2, input))
        out_encoder_4 = self.encoder_4(self.comb4(out_encoder_3, input))
        out_encoder_5 = self.encoder_5(self.comb5(out_encoder_4, input))
...
if __name__ == '__main__':
    model = Model_MTAS1(in_ch=9, out_ch=1,
